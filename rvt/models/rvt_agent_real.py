...
from rvt.libs.M2T2.demo_rlbench import load_data
from m2t2.config import get_cfg_defaults
...
class RVTAgent:
    ...
    def update(
        ...
        # keys = replay_sample.keys()  # 这将返回所有的键
        # front_mask = replay_sample["front_mask"]
        # sample
        ...
        # model_input = load_data(replay_sample, self.cameras, self.scene_bounds)
        ...
        with torch.no_grad():
            ...
            # _________________add_feats____________________
            # cfg = get_cfg_defaults()
            # print(len(pc))
            # print(len(img_feat))
            # print(lang_goal_embs.shape)
            # print(pc[0].shape)
            # print(img_feat[0].shape)
            # exit()
            # model_inputs = load_data(pc, img_feat, lang_goal_embs) 
            # embedding, outputs, last_feats = self.detect.tmp_infer(model_inputs, self._device)
        ...
        with autocast(enabled=self.amp):
            ...
            out = self._network(
                ...
                # detect_feats=outputs["grasping_masks"],
        ...
        if backprop:
            ...
            if torch.isnan(total_loss):
                ...
                # else:
                #     total_loss=torch.rand(1) * 5.
                #     total_loss= torch.tensor(total_loss, requires_grad=True).to(collision_loss.device)
                #     print(total_loss)
    ...
    def act(
        ...
        # _________________add_feats____________________
        model_inputs = load_data(pc, img_feat, lang_goal_embs) 
        # embedding, outputs, last_feats = self.detect.tmp_infer(model_inputs, self._device) # Marco, M2T2
        # TODO: Vectorize
        pc_new = []
        rev_trans = []
        for _pc in pc:
        ...
        out = self._network(
            ...
            # detect_feats=outputs["grasping_masks"], # Marco
            # last_feats=last_feats,
        )
