...
from rvt.libs.M2T2.demo_rlbench import get_model
...
from rvt.utils.real_world_const import (
    CAMERAS,
    DATA_FOLDER,
    IMAGE_SIZE,
    SCENE_BOUNDS,
...
def get_model_path(model_path, i):
    
    # print(model_path[:-6] + f"{i}.pth")
    # exit()
    return model_path[:-6] + f"{i}.pth"
...
def _eval(args):
    ...
    begin_idx = get_model_index(model_paths[0])
    n = 1
    for i in range(n):
        model_idx = begin_idx+i
        model_path = get_model_path(model_paths[0], model_idx)
        ...
        with open(os.path.join(agent_eval_log_dir, "eval_results.csv"), "a") as csv_fp:
            fieldnames = ["task", "success rate", "length", "total_transitions"]
            csv_writer = csv.DictWriter(csv_fp, fieldnames=fieldnames)
            csv_results = {"task": sum(scores)/len(scores),
                           "success rate": sum(scores)/len(scores),
                           "length": sum(scores)/len(scores),
                           "total_transitions": sum(scores)/len(scores)}
            csv_writer.writerow(csv_results)
            
            
        print(f"model {model_path}, scores {scores}")
        print(f"model {model_path}, avg_scores {sum(scores)/len(scores)}")
        task_scores = {}
