...
def get_pc_img_feat(obs, pcd, bounds=None):
    ...
    # mask_feat = torch.cat([p.permute(0, 2, 3, 1).reshape(bs, -1, 3) for p in masks], 1)
...
def move_pc_in_bound(pc, img_feat, bounds, no_op=False):
    ...
    pc = [pc[i, ~_inv_pnt] for i, _inv_pnt in enumerate(inv_pnt)]
    img_feat = [img_feat[i, ~_inv_pnt] for i, _inv_pnt in enumerate(inv_pnt)]
    # mask_feats = [mask_feats[i, ~_inv_pnt] for i, _inv_pnt in enumerate(inv_pnt)]
    return pc, img_feat
    ...
    if not only_epoch:
        ...
        try:
            ...
            try:
                ...
                # params_to_delete = [k for k in checkpoint["model_state"].keys() if k.startswith("mvt2.detect_model")]

                # for param in params_to_delete:
                #     del checkpoint["model_state"][param]
                # params_to_delete = [k for k in checkpoint["model_state"].keys() if k.startswith("mvt2.seg_model")]

                # for param in params_to_delete:
                #     del checkpoint["model_state"][param]

                # for param in checkpoint["model_state"].keys():
                #     print(param)
                # exit()
