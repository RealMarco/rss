...
class MVT(nn.Module):
    def __init__(
        ...
        get_mamba = lambda: PreNorm(
            attn_dim,
            Mamba(
            # This module uses roughly 3 * expand * d_model^2 parameters
            d_model=attn_dim, # Model dimension d_model
            d_state=16,  # SSM state expansion factor
            d_conv=4,    # Local convolution width
            expand=2,    # Block expansion factor
        )
        )
        get_mamba_ffn = lambda: PreNorm(attn_dim, FeedForward(attn_dim))
        get_mamba, get_mamba_ffn = map(cache_fn, (get_mamba, get_mamba_ffn))
        # self-attention layers
        ...
        for _ in range(attn_depth):
            self.mamba_layers.append(
                nn.ModuleList([get_mamba(**cache_args), get_mamba_ffn(**cache_args)])
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # cross attention
        for self_attn, self_ff in self.mamba_layers:
            x = self_attn(x) + x
            x = self_ff(x) + x
