...
from rvt.mvt.mvt_single_real import MVT as MVTSingle
...
class MVT(nn.Module):
    ...
    def verify_inp(
        ...
        if self.add_proprio:
            ...
            assert bs == bs3
            assert (
                x3 == self.proprio_dim # self.proprio_dim ==4
        ...
        with torch.no_grad():
            if self.training and (self.img_aug_2 != 0):
                for x in img_feat:
                    ...
                    x = x + noise
            img = self.render( # synthesize 10 virtual view images 
                pc=pc,
                img_feat=img_feat,
                img_aug=img_aug,
                mvt1_or_mvt2=True,
